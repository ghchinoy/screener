...
- **SwiftData Persistence:** The app uses `@Model` to persist Gemini summaries, comments, and technical metadata.
  - **Important:** If you change `VideoMetadata.swift` during development, automatic migration will fail and crash the app. You MUST run `make reset-db` to clear the local database before launching the app again.
